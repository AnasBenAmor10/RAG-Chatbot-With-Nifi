...
from nifiapi.documentation import (
...
class ChunkDocument(FlowFileTransform):
    ...
    def split_docs(self, context, flowfile, documents):
        ...
        if strategy == SPLIT_BY_CHARACTER:
            text_splitter = CharacterTextSplitter(
                separator=context.getProperty(self.SEPARATOR)
            ...
            separator_text = (
                context.getProperty(self.SEPARATOR)
                .evaluateAttributeExpressions(flowfile)
                .getValue()
            )
    ...
    def to_json(self, docs) -> str:
        ...
        for i, doc in enumerate(docs):
            ...
            json_doc = json.dumps(
                {TEXT_KEY: doc.page_content, METADATA_KEY: doc.metadata}
            )
    ...
    def transform(self, context, flowfile):
        ...
        return FlowFileTransformResult(
            "success", contents=output_json, attributes=attributes
        )
