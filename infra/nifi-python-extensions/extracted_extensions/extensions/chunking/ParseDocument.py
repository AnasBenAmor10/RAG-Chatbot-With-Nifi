...
class ParseDocument(FlowFileTransform):
    ...
    PDF_INFER_TABLE_STRUCTURE = PropertyDescriptor(
        ...
        required=True,
        dependencies=[
            PropertyDependency(PDF_PARSING_STRATEGY, PARSING_STRATEGY_HIGH_RES)
        ],
    )
    ...
    def create_docs(self, context, flowFile):
        ...
        input_format = (
            context.getProperty(self.INPUT_FORMAT)
        ...
        if input_format == PLAIN_TEXT:
            return [
                Document(
                    page_content=flowFile.getContentsAsBytes().decode("utf-8"),
                    metadata=metadata,
                )
            ]
        ...
        if input_format == HTML:
            ...
            infer_table_structure = context.getProperty(
                self.PDF_INFER_TABLE_STRUCTURE
            ).asBoolean()
            ...
            languages = self.get_languages(
                context.getProperty(self.LANGUAGES).getValue()
            )
